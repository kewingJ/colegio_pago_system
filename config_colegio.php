...
<html class="fixed">
	<head>
		...
		<title>Configuracion Colegio</title>
	...
	<body>
                                            ...
                                            <?php
                                                ...
                                                while($rowUser = mysqli_fetch_array($queryUser))
                                                {
                                                    echo '
                                                            <tr>
                                                                <td>'.$cont.'</td>
                                                                <td>'.$rowUser['NOMBRECOLEGIO'].'</td>
                                                                <td>'.$rowUser['DIRECCION'].'</td>
                                                                <td>'.$rowUser['TELEFONOS'].'</td>
                                                                <td class="text-center">
                                                                    <div class="dropdown">
                                                                        <button style="font-size: small;" type="button" class="btn btn-primary dropdown-toggle" data-bs-toggle="dropdown" aria-expanded="false">
                                                                            Opciones
                                                                        </button>
                ...
                <div class="modal-dialog" role="document">
                    <div class="modal-content">
                        ...
                        <div class="modal-body">
                            <div id="content_dynamic_edit_colegio">
        ...
        <script type="text/javascript">
            $(document).ready(function(){
                function initLogoDropzoneOnce() {
                    if (!window.Dropzone) return;
                    Dropzone.autoDiscover = false;

                    var el = document.getElementById('logoDropzone');
                    if (!el) return;

                    // destruye instancia previa si reabren el modal
                    if (window._logoDZ && typeof window._logoDZ.destroy === 'function') {
                        try { window._logoDZ.destroy(); } catch(e){}
                    }

                    var uploadUrl = el.getAttribute('data-upload-url') || 'controller/upload_logo.php';

                    window._logoDZ = new Dropzone(el, {
                        url: uploadUrl,
                        paramName: "file",
                        maxFiles: 1,
                        maxFilesize: 3,
                        ...
                        // 🔎 Validación de dimensiones mínimas (cliente)
                        accept: function(file, done) {
                            ...
                            img.onerror = function() {
                                done("No se pudo leer la imagen.");
                            };
                        ...
                        init: function () {
                            this.on("success", function(file, resp){
                                ...
                                if (resp && resp.status === 'ok' && resp.url) {
                                    $('#logo_colegio').val(resp.url);
                                    $('#logo-preview').show().html(
                                        '<img src="'+resp.url+'" style="width:120px;height:120px;object-fit:contain;border:1px solid #ddd;padding:4px;border-radius:6px;background:#fff;">'
                                    );
                ...
                $(document).on('click', '.btn1', function(e){
                    ...
                    // alert(id_email);
                    $('#content_dynamic_edit_colegio').html('');  
                    $.ajax({
                        url: 'controller/get_colegio_edit.php',
                        type: 'POST',
                    ...
                    .done(function(data){  
                        $('#content_dynamic_edit_colegio').html('');    
                        $('#content_dynamic_edit_colegio').html(data); // mostrar la data
                        initLogoDropzoneOnce(); // inicializar Dropzone
                    })
                    .fail(function(){
                        $('#content_dynamic_edit_colegio').html('<i class="glyphicon glyphicon-info-sign"></i> Something went wrong, Please try again...');
                    });
                });
                ...
                $(document).on('click', '#btnA', function(e){
                    ...
                    $.ajax({
                        type: 'POST',
                        url: 'controller/a_colegio.php',
                        data: $('.FormU').serialize(),
                        success: function(data) {
                            if (data == 'bien') {
                                not3();
                                setTimeout("location.href = 'config_colegio.php'",3000);
                            } else {
                                not2();
                            }
                        }
        ...
        <script type="text/javascript">
            $(document).ready(function(){
                $(document).on('click', '#btnGuardarCorreo', function(e){
                    ...
                    $.ajax({
                        type: 'POST',
                        url: 'controller/send_email_test.php',
                        data: $('.FormAlumno').serialize(),
                        success: function(data) {
                            if (data == 'bien') {
                                not3();
                                setTimeout("location.href = 'config_email.php'",3000);
                            } else {
                                not2();
                            }
                        }
