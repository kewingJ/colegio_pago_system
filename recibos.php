...
<html class="fixed">
	...
	<body>
                                    ...
                                    <table class="table table-sm table-bordered table-striped small mb-0" id="example1">
										...
										<thead>
											<tr>
                                                ...
                                                <th>Fecha recibo</th>
                                                <th class="text-center">Estado recibo</th>
                    ...
                    <div class="modal-content">
                        ...
                        <div class="modal-body">
                            <form id="Form2" class="FormC" action="" method="" autocomplete="off">
                                <h4 class="text-center">¿Esta seguro de anular el recibo?</h4>
                                <input type="hidden" name="id_recibo" id="id_recibo">
                                    
                                <div class="text-center col-md-12">
                                    <button class="btn btn-default" data-dismiss="modal" type="reset"><i class="fa fa-close"></i> Cancelar</button>
                                    <button class="btn btn-danger" type="button" id="btnEliminar"><i class="fa fa-trash"></i> Anular</button>
                                </div>
                            </form>
                        </div>
                        <div class="modal-footer">
        ...
        <script>
            $(function () {
                $('#example1').DataTable({
                    ...
                    "serverSide": true,
                    "order": [[1, 'desc']],
                    "ajax": "ajax_table/ajax_table_recibos.php",
                    "createdRow": function ( row, data, index ) {

                        if (data[4] == 1) {
                            $('td', row).eq(4).addClass("text-center");
                            $('td', row).eq(4).html('<span class="badge badge-success">Pagado</span>');
                        } else {
                            $('td', row).eq(4).addClass("text-center");
                            $('td', row).eq(4).html('<span class="badge badge-danger">Anulado</span>');
                        }

                        //
                        if (data[5]) {
                            $('td', row).eq(5).addClass("text-center");
                            //$('td', row).eq(4).html('<div class="btn-group"><button type="button" class="btn btn-primary">Opciones</button><button type="button" class="btn btn-primary dropdown-toggle" data-toggle="dropdown"><span class="caret"></span><span class="sr-only">Toggle Dropdown</span></button><ul class="dropdown-menu" role="menu"><li><a href="#0" data-toggle="modal" data-target="#ModalEditar" data-id="'+data[4]+'" class="btn1"><i class="fa fa-print"></i> Reimpresión</a></li><li><a href="#0"data-id="'+data[4]+'" class="btn2"><i class="fa fa-trash"></i> Anular recibo</a></li></ul></div>');
                            $('td', row).eq(5).html(`
                                <div class="dropdown">
                                    ...
                                    <ul class="dropdown-menu">
                                        <li>
                                            <a class="dropdown-item btn1" href="#" data-id="${data[5]}" data-bs-toggle="modal" data-bs-target="#ModalEditar">
                                                <i class="fa fa-print"></i> Reimpresión
                                            </a>
                                        </li>
                                        <li>
                                            <a class="dropdown-item btn2" href="#" data-id="${data[5]}">
                                                <i class="fa fa-trash"></i> Anular recibo
                                            </a>
                                        </li>
                                    </ul>
                                </div>
        ...
        <!-- Mostrar factura para reimprimir -->
        <script type="text/javascript">
            $(document).ready(function(){
                // primer paso mostrar datos
                $(document).on('click', '.btn1', function(e){
                    e.preventDefault();
                    var id_recibo = $(this).data('id');
                    // alert(id_usuario);
                    $('#content_dynamic_edit_arancel').html('');  
                    $.ajax({
                        url: 'controller/get_data_recibo.php',
                        type: 'POST',
        ...
        <script type="text/javascript">
            $(document).ready(function(){
                ...
                $(document).on('click', '#btnEliminar', function(e){
                    ...
                    $.ajax({
                        type: 'POST',
                        url: 'controller/e_recibo.php',
                        data: 'id_recibo='+id_recibo,
                        success: function(data) {
                            if (data == 'bien') {
                                not4();
                                setTimeout("location.href = 'recibos.php'",2000);
                            } else {
                                not5();
                            }
                        }
