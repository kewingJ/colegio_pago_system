...
<html class="fixed">
	...
	<body>
		<section class="body">
							...
							<section class="card">
								<header class="card-header">
                                    ...
                                    <form id="Form1" class="FormNuevo" action="" method="" autocomplete="off">
                                            <div class="row">
                                                <div class="col-md-3">
                                                    <label>Seleccione la fecha del reporte</label>
                                                    <!-- <input type="text" class="form-control pull-right" id="datepicker" name="fechaRecibo" id="fechaRecibo"> -->
								...
								<div class="card-body">
                                    <div class="col-md-12" id="reporte_facturas"></div>
                                    <!-- Acciones de cierre de caja -->
									<div id="accionesCaja" class="mt-3 d-none">
										<div class="btn-group" role="group" aria-label="Acciones cierre de caja">
											...
											if($_SESSION['tipo_usuario'] == 2){
											?>
											<button type="button" id="btnEnviarGerente" class="btn btn-primary">
												<i class="fa fa-envelope"></i> Enviar a gerente
											</button>
											<button type="button" id="btnEnviarOtro" class="btn btn-primary">
												<i class="fa fa-envelope"></i> Enviar a otro correo
											</button>
											<button type="button" id="btnEnviarWhatsapp" class="btn btn-success">
									...
									<!-- Form oculto para descargar PDF por POST -->
									<form id="formDescargarPdf" method="POST" action="controller/cierre_caja_pdf.php" target="_blank" class="d-none">
										<input type="hidden" name="rango" id="rangoPdf">
									</form>
								</div>
							</section>
						</div>
			...
			<!-- Modal enviar a otro correo -->
			<div class="modal fade" id="modalEnviarOtro" tabindex="-1" role="dialog" aria-labelledby="modalEnviarOtroLabel" aria-hidden="true">
				<div class="modal-dialog" role="document">
					<div class="modal-content">
					<div class="modal-header">
						<h5 class="modal-title" id="modalEnviarOtroLabel">Enviar cierre de caja</h5>
						<button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Cerrar"></button>
					</div>
					<div class="modal-body">
						<form id="formEnviarOtro" autocomplete="off">
							<div class="mb-3">
								<label for="emailOtro" class="form-label">Correo electrónico</label>
								<input type="email" class="form-control" id="emailOtro" name="emailOtro" placeholder="correo@ejemplo.com">
							</div>
						</form>
					</div>
					<div class="modal-footer">
						<button type="button" class="btn btn-default" data-bs-dismiss="modal">Cancelar</button>
						<button type="button" class="btn btn-primary" id="btnConfirmEnviarOtro">
		...
		<!-- Moment.js y Daterangepicker.js -->
        ...
        <!-- al seleccionar fecha de reporte -->
		<script>
			// variable global para recordar el rango actual
			let rangoCaja = '';

			$(document).ready(function() {
				$('#rangoFechas').on('apply.daterangepicker', function(ev, picker) {
					rangoCaja = picker.startDate.format('DD/MM/YYYY') + '|' + picker.endDate.format('DD/MM/YYYY');

					$.ajax({
						url: 'controller/get_reporte_facturas.php',
						type: 'POST',
						data: { rango: rangoCaja },
						dataType: 'html',
						success: function(data){
							$('#reporte_facturas').html(data);
							// mostrar botones de acciones
							$('#accionesCaja').removeClass('d-none');
						},
						error: function(){
							$('#reporte_facturas').html('Error cargando el reporte.');
							$('#accionesCaja').addClass('d-none');
						}
					});
				});
			});
		...
		<script>
			$(document).ready(function(){
				...
				$('#btnDescargarPdf').on('click', function(){
					if (!rangoCaja) {
						not2(); // “Algunos campos están vacíos”
						return;
					}
					$('#rangoPdf').val(rangoCaja);
					$('#formDescargarPdf').submit(); // abrirá el PDF en nueva pestaña / descarga
				});

				// 2) Enviar a gerente
				$('#btnEnviarGerente').on('click', function(){
					if (!rangoCaja) {
						not2();
						return;
					}
					...
					$.ajax({
						url: 'controller/enviar_cierre_gerente.php',
						type: 'POST',
						data: { rango: rangoCaja },
						success: function(resp){
							if (resp.trim() === 'bien') {
								notif({
									msg: "Se envio correctamente al gerente",
									type: "success",
									position: "center"
								});
							} else {
				...
				// 3) Enviar a otro correo
				$('#btnEnviarOtro').on('click', function(){
					if (!rangoCaja) {
						not2();
						return;
					}
					$('#emailOtro').val('');
					$('#modalEnviarOtro').modal('show');
				});

				$('#btnConfirmEnviarOtro').on('click', function(){
					const email = $('#emailOtro').val().trim();
					if (!email) {
						not2();
						return;
					}
					...
					$.ajax({
						url: 'controller/enviar_cierre_otro.php',
						type: 'POST',
						data: { rango: rangoCaja, email_otro: email },
						success: function(resp){
							if (resp.trim() === 'bien') {
								$('#modalEnviarOtro').modal('hide');
								notif({
									msg: "Se envio correctamente al correo indicado",
									type: "success",
									position: "center"
								});
							} else {
				...
				// 4) Enviar cierre de caja por WhatsApp
				$('#btnEnviarWhatsapp').on('click', function(){
					...
					$.ajax({
						url: 'controller/enviar_cierre_whatsapp.php',
						type: 'POST',
						data: { rango: rangoCaja },
						success: function(resp){
							if (resp.trim() === 'bien') {
								notif({
									msg: "Se envió correctamente el cierre de caja por WhatsApp",
									type: "success",
									position: "center"
								});
							} else {
        ...
        <script>
            $(document).ready(function() {
                $("#btnImprimirRecibo").click(function() {
                    var contenidoRecibo = $("#reporte_facturas").html();
