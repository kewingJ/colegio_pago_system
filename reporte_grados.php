...
<html class="fixed">
	<head>
		...
		<title>Reporte por grados</title>
	...
	<body>
                                    ...
                                    <form id="Form1" class="FormFiltro" action="" method="" autocomplete="off">
                                        <div class="row">
                                                <div class="col-md-3">
                                                    ...
                                                    <select id="dropdown1" data-plugin-selectTwo2 style="width: 100%" name="dropdown1">
                                                ...
                                                <div class="col-md-3">
                                                    ...
                                                    <select id="dropdown2" data-plugin-selectTwo2 style="width: 100%" name="dropdown2">
                                                ...
                                                <div class="col-md-3">
                                                    <label>Grado</label>
                                                    <select id="dropdown3" data-plugin-selectTwo2 style="width: 100%" name="dropdown3">
                                                ...
                                                <div class="col-md-3">
                                                    <label>Seccion</label>
                                                    <select id="dropdown4" data-plugin-selectTwo2 style="width: 100%" name="dropdown4">
       ...
       <!-- script de tabla -->
        <script>
            $(function () {				
                const table =$('#example1').DataTable({
                    ...
                    "serverSide": true,
                    "order": [[2, 'desc']],
                    "ajax": "ajax_table/ajax_table_reporte_alumnos.php",
				...
				function actualizarTotalFiltrado() {
						const year_lectivo   	= $('#dropdown1').val() || '';
						const nivel_escolar   	= $('#dropdown2').val() || '';
						const grado           	= $('#dropdown3').val() || '';
						const seccion         	= $('#dropdown4').val() || '';
						const q            	 	= table.search() || '';

						var numeroDeColumnas = 14;

						$.getJSON('ajax_table/sum_reporte_alumnos_grado.php', {
							year_lectivo, nivel_escolar, grado, seccion, q
						}).done(function (resp) {
							...
							$(api.column(numeroDeColumnas).footer()).html('C$ ' + formateado);
						}).fail(function () {
							// opcional: deja vacío o muestra 0
							const api = $('#example1').DataTable();
							$(api.column(numeroDeColumnas).footer()).html('C$ 0.00');
						});
				}

				// Cuando el usuario escribe en el buscador global, recalculamos el total
				table.off('search.dt.recalc').on('search.dt.recalc', function () {
        ...
        <script>
            $(document).ready(function () {
				// Reemplaza tu handler actual del botón "Descargar PDF" por este:
				$(document).on('click', '#btnDescargarDatos', function (e) {
					...
					const params = new URLSearchParams({
						anio:          $('#dropdown1').val() || '',
						nivel_escolar: $('#dropdown2').val() || '',
						grado:         $('#dropdown3').val() || '',
						seccion:       $('#dropdown4').val() || '',
						q:             table.search() || ''   // <-- buscador global
					});

					window.open('reports/reporte_mora_pdf.php?' + params.toString(), '_blank');
				});
            });
        </script>
