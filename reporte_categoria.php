								...
								<header class="card-header">
                                    ...
                                    <form id="Form1" class="FormFiltro" action="" method="" autocomplete="off">
                                        <div class="row">
                                                    ...
                                                    <div class="col-md-3">
                                                        <label>Categoria pago</label>
                                                        <select data-plugin-selectTwo2 class="form-control" style="width: 100%" id="id_categoria" name="id_categoria">
                                                            <option value="">Categoria pago</option>
                                                            <?php
                                                                $queryData = mysqli_query($link,"SELECT * FROM tbl_categoriapago ORDER BY Id DESC");
                                                                while($rowData = mysqli_fetch_array($queryData)){
                                                    ...
                                                    <div class="col-md-3">
                                                        ...
                                                        <select data-plugin-selectTwo2 class="form-control" style="width: 100%" id="id_concepto" name="id_concepto">
                                                            <option value="">Concepto pago</option>
                                                        </select>
                                                    </div>

                                                    <div class="col-md-2">
                                                        <label>Mes pago</label>
                                                        <select data-plugin-selectTwo2 class="form-control" style="width: 100%" id="id_mes" name="id_mes">
                                                            <option value="">Meses</option>
                                                            <?php
                                                                $queryData = mysqli_query($link,"SELECT * FROM tbl_mes");
                                                                while($rowData = mysqli_fetch_array($queryData)){
                                                                    $mesFormato = ucfirst(strtolower($rowData['MES']));
                                                                    echo '<option value="'.$rowData['ID'].'">'.$mesFormato.'</option>';
                                                                }
                                                            ?>
                                                        </select>
                                                    </div>
        ...
        <script>
            $(function () {
                $('#example1').DataTable({
                    ...
                    "ajax": "ajax_table/ajax_table_reporte_categoria.php",
                ...
                $('#id_categoria').on('change',function(){
                    ...
                    $('#id_concepto').html('');  
                    $.ajax({
                        url: 'controller/get_concepto_list_dos.php',
                        type: 'POST',
                ...
                $('#id_mes').on('change',function(){
                    ...
                    // alert(valorBusqueda);
                    table.columns(2).search(valorBusqueda).draw();
                    actualizarTotalFiltrado();
                });
            });
            ...
            function actualizarTotalFiltrado() {
                ...
                const nivel     = $('#id_nivel').val() || '';
                const categoria = $('#id_categoria').val() || '';
                const concepto  = $('#id_concepto').val() || '';
                const mes       = $('#id_mes').val() || ''; // asumiendo value = 1..12

                $.getJSON('ajax_table/sum_reporte_categoria.php', {
                    anio, nivel, categoria, concepto, mes
                }).done(function (resp) {
                    ...
                    $(api.column(7).footer()).html('C$ ' + formateado);
                }).fail(function () {
                    // opcional: deja vacío o muestra 0
                    const api = $('#example1').DataTable();
                    $(api.column(7).footer()).html('C$ 0.00');
                });
            }
        </script>
