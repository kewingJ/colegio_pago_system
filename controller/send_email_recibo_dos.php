...
if (!empty($_POST['email_alumno']) &&
    ...
    // guardar el correo al alumno
    $queryUpdate = mysqli_query(
        $link,
        "UPDATE tbl_alumnos SET EMAIL = '$email_alumno' WHERE IDALUMNO = '$id_alumno_email'"
    ...
    if ($ok) {
        ...
        foreach ($trs as $tr) {
            ...
            if ($tds->length == 2) {
                ...
                $valorNum = is_numeric(str_replace(',', '.', $valorTxt))
                    ? (float)str_replace(',', '.', $valorTxt)
                    : null;
    ...
    // Render para la tabla de líneas (prefijo SIEMPRE en la columna de importes) con zebra
    $rowsHtml = '';
    $idx = 0;
    ...
    // fila total en negrita (si lo tenemos)
    if ($montoTotal !== null) {
        $rowsHtml .= '
          <tr class="total-row">
            <td>Total pagado</td>
    ...
    // ===== 1) Crear PDF con mismo diseño que el de WhatsApp =====
    ...
    // tipografías Titillium
    $pdf->SetFont('titilliumweb', 'B', 16);

    // Título centrado (nombre del colegio)
    ...
    <style>
      ...
      .muted { color:#475569; font-size:11px; } /* un poco más oscuro */
      ...
      .row-alt { background-color:#f1f5f9; } /* zebra */
    ...
    $ruta_guardado  = __DIR__ . '/recibos/' . $nombre_archivo;
